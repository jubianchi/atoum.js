var atoum = require('..'),
    console = require('console'),
    exception = require('./test/exception'),
    test = module.exports = function test(testClass) {
        this.class = testClass;
        this.buffer = '';
        this.testMethods = 0;
        this.failedMethods = 0;
        this.errorMethods = 0;
        this.errors = '';
        this.usage = {};
    };

test.prototype = {
    run: function(stdout) {
        var usageBefore = process.memoryUsage();

        stdout.write('[');

        try {
            methods = this.getMethods();

            for(var name in methods) {
                this.testMethods++;

                try {
                    methods[name].call(this);
                    stdout.write('\033[32mS\033[0m');
                } catch(exception) {
                    if(exception.name === 'Failure') {
                        this.failedMethods++;
                        stdout.write('\033[31mF\033[0m');
                        this.buffer += '\033[0;31m' + exception.message + '\033[0m\n';
                        this.buffer += exception.stack;
                    } else {
                        this.errorMethods++;
                        stdout.write('\033[31mE\033[0m');
                        this.errors += '\033[0;31m' + exception.message + '\033[0m\n';
                        this.errors += exception.stack;
                    }
                }
            }
        } catch(exc) {
            if(exc instanceof exception) {
                this.failedMethods++;
                stdout.write('\033[31mX\033[0m');
                this.buffer += '\033[0;31m' + exc.message + '\033[0m\n';
                this.buffer += exc.stack;
            }
        }

        stdout.write(']\n');

        var usageAfter = process.memoryUsage();
        for(var key in usageBefore) {
            var use = 0;
            if(usageBefore.hasOwnProperty(key) && usageAfter.hasOwnProperty(key)) {
                use = (usageAfter[key] - usageBefore[key]) / 1024;
            }

            this.usage[key] = (use > 0 ? use : 0);
        }
    },

    getMethods: function() {
        try {
            var unit = atoum.require(this.class),
                methods = [];

            for(method in unit) {
                if(unit.hasOwnProperty(method)) {
                    methods[method] = unit[method];
                }
            }
        } catch(exc) {
            throw new exception(exc.message);
        }

        return methods;
    }
};
