...
test.prototype = {
        ...
        try {
            ...
            for(key in methods) {
                if(methods.hasOwnProperty(key) && typeof methods[key] === 'function') {
                    ...
                    this.dispatcher.emit('testMethodStart', this, key);

                    try {
                        methods[key].call(this);

                        if(this.dispatcher) {
                            this.dispatcher.emit('testMethodSuccess', this, key);
                        }
                    } catch(exception) {
                        if(exception instanceof failure) {
                            this.score.addFailure(key, exception);

                            if(this.dispatcher) {
                                this.dispatcher.emit('testMethodFailure', this, key);
                            }
                        } else {
                            exception.stack = (exception.stack || '').split('\n').slice(1).join('\n');
                            this.score.addException(key, exception);

                            if(this.dispatcher) {
                                this.dispatcher.emit('testMethodException', this, key);
                            }
                        }
                    }

                    this.dispatcher.emit('testMethodStop', this, key);
                }
            }
        } catch(error) {
            error.stack = error.stack.split('\n').slice(1).join('\n');
            this.score.addError(this.class, error);
    ...
    getMethods: function () {
        ...
        for(method in unit) {
            if(unit.hasOwnProperty(method) && /^test[A-Z_\-]/.exec(method)) {
